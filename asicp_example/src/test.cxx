...
void test_asicp(void)
{
    ...
    asicp(X, Y, threshold, maximum_iterations, asopa_threshold, false, rotations, Q, A, t, indices, RMSE, false);
	...
	std::cout << "Original Error of Y - X" << std::endl;
	std::cout << "RMSE:" << sqrt((Y(Eigen::all, indices) - X).squaredNorm()/n) << std::endl;
		
	std::cout << "Actual Error of Y - (QAX+t)" << std::endl;
	std::cout << "RMSE:" << sqrt((Y(Eigen::all, indices) - ((Q * (A * X)).colwise() + t)).squaredNorm()/n) << std::endl;
}
